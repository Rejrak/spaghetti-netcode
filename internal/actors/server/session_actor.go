...
func (s *session) readUserAttributes(c context.Context, address string) {
	repo, err := sqlite.Open("./authblock.db")
	if err != nil {
		slog.Info("sqlite open error: %v", err)
	}
	s.repo = repo
	userAttrs, updated, success, err := s.repo.GetAttrs(c, address)
	slog.Info("Auth Request", "address", address, "attrs", userAttrs, "updated", updated, "success", success, "err", err)
}
...
func (s *session) Receive(c *actor.Context) {
	...
	case *packets.AuthMessage:
		s.readUserAttributes(c.Context(), msg.Address)
